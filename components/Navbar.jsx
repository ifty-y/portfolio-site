...
const NavItems = ({ isNavOpen, setIsNavOpen }) => {
	const { theme } = useTheme(); // Moved this line here to access theme
...
const Navbar = () => {
	...
	const { theme } = useTheme(); // Get the current theme here
	...
	return (
		<>
			<nav
				ref={navRef}
				className={`navbar px-5 md:px-24 w-screen fixed transition-colors ease duration-600 ${
					isNavOpen
				...
				<div>
					<h1
						className={`text-2xl ml-2 md:ml-0 transition-colors ease duration-600 ${
				...
				<div className="flex flex-row items-center">
					<button
						className={`burger button flex flex-col justify-center items-center space-y-1.5 transition-colors ease duration-300 ${theme === "dark" ? "text-white" : "text-black"}`}
						onClick={toggleNav}>
						<div
							className={`w-10 h-1 rounded-full transition-all ease duration-600 ${
								isNavOpen ? "rotate-45 translate-y-[2px] bg-white" : (theme === "dark" ? "bg-white" : "bg-black")
							}`}></div>
						<div
							className={`w-10 h-1 rounded-full transition-all ease duration-600 ${
								isNavOpen ? "-rotate-45 -translate-y-2 bg-white" : (theme === "dark" ? "bg-white" : "bg-black")
							}`}></div>
					</button>
				</div>
			</nav>
